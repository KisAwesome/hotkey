...
class ExpectedArgument(CodeError):
    pass


class InvalidButton(CodeError):
    pass
...
__all__ = ['bind', 'delay', 'cmd', 'click', 'typewrite',
           'website', 'press', 'open', 'move_mouse', 'loop', 'endloop', 'mode', 'clipboard']
...
var_types = {'time':str,'date':str,'full_date':str}
    ...
    for i in scriptlines:
        ...
        if not command in __all__:
            raise FunctionNonExistant(
                f'Function does not exist {command} Line {iter_line}')
    ...
    for i in scriptlines:
        ...
        if command == 'click':
            if not ARGUMENT in __buttons__:
                raise InvalidButton(f'Line {index+1}: invalid mouse button')
        ...
        if command == 'mode':
            if not ARGUMENT in __modes__:
                raise InvalidMode('Invalid Mode')
            MODE = ARGUMENT
            continue
